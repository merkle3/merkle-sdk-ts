...
class PrivatePool {
  ...
  auctions(): TypedEventEmitter<PrivatePoolStreamEvents> {
    ...
    async function connect() {
      const closed = new Promise((resolve) => {
        // open the websocket
        const conn = new WebSocket("wss://mempool.merkle.io/stream/auctions");
  ...
  async bid(txHash: string, backrun: string): Promise<string> {
    const body = {
    ...
    const res = await fetch(`https://mempool.merkle.io/relay`, {
      method: "POST",
  ...
  async sendRaw(transaction: string, opts?: SendTransactionOptions) {
    ...
    const res = await fetch(`https://mempool.merkle.io/transactions`, {
      method: "POST",
      body: JSON.stringify(body),
      headers,
    });
